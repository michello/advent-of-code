...
def get_num_rolls_accessible(grid):
...
def solution():
    grid = get_grid()
    num_rolls_accessible = get_num_rolls_accessible(grid)
    num_rolls_rid = 0

    while num_rolls_accessible:
        for row in range(len(grid)):
            for col in range(len(grid[row])):
                curr_elem = grid[row][col]
                if curr_elem == "@" and can_lift_roll(grid, row, col):
    ...
    return num_rolls_rid

print(solution()) # 1537 for input2.csv, 8707 for input.csv